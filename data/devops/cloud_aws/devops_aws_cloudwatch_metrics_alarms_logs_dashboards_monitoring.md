...
- **Metric filters** — create CloudWatch metrics from log patterns (e.g., count ERRORs/min)
- **Subscription filters** — stream logs to Lambda, Elasticsearch, Kinesis, S3
...
**Rule of thumb:** CloudWatch for AWS-native monitoring (free tier covers basics). Add CloudWatch Agent for memory/disk. Use Log Insights for ad-hoc log queries. Alarm on user-facing symptoms (5XX rate, latency), not just resource metrics (CPU). For deep APM, consider Datadog or Grafana Cloud alongside CloudWatch.
