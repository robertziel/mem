### External API Resilience (Timeout, Retry, Circuit Breaker, Bulkhead)
...
**7. Service wrapper pattern — encapsulate integration:**
```ruby
...
class OrdersController < ApplicationController
  def create
    ...
    PaymentGateway.charge(@order.total)   # user waits
...
**Rule of thumb:** Always set timeouts. Retry with exponential backoff + jitter (never retry in a tight loop). Use circuit breakers for critical services to fail fast. Make mutations idempotent with idempotency keys. Isolate failures with bulkhead (separate pools/queues per service). Wrap each API in a dedicated service class. Offload to background jobs so users get instant responses.
